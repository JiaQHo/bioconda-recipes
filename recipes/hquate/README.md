
# Getting Started
```sh
path/to/bin/HQuaTE -b <sorted.BAM> -g <TE.gff> 
```

# Users' Guide
HQuaTE is a tool for **quantifying the transposable elements (TEs) expression in read-specific resolution by short read RNA-seq**. HQuaTE is implemented in **Bash only** till now, and requires no installation. However, other softwares that are part of the tool are required (see Dependencies).

## Installation
### Anaconda
HQuaTE is available on conda and can be installed using conda. 
```sh
conda install jiaqiang_987::HQuaTE
```

### Dependencies
If conda does not successfully install, HQuaTE could be obtained from github (https://github.com/PengjuZ/HQuaTE). Then download the required softwares as following, and add them to the `PATH` variable. 

* Basic quantificion
  * bedtools >=2.30.0,<2.35.0
  * samtools >=1.15,<1.20
  
* Advanced option for anomalous-expression detection 
  * python python >=3.9,<3.13
  * scikit-learn ==1.5.1
  * numpy ==2.1.0
  * pandas ==2.2.2

### Test files
Two files `examples/test.bam` & `examples/testTE.gff` are provided within HQuaTE for testing its operation. Once HQuaTE and its dependences are in your `PATH` variable, you can execute it as follows. Normally, this process takes in seconds. By default, a output file `test.bam_TE` would be generated in the current directory. 
```sh 
HQuaTE -b test.bam -g testTE.gff
```

## General usage
Once HQuaTE and its dependences are in your `PATH` variable, you can execute it as:
```sh
HQuaTE -b <sorted.BAM> -g <TE.gff> 
```
Important considerations:
* `-b` <b>BAM file must be sorted.</b>
* `-g` Notably, our testing has been conducted solely with the <b>GFF file from RepeatMaske.</b> Thus, the TE annotation file shall be **generated by RepeatMasker, or as follows:** (1) Column 1 (chr): Specifies the chromosome name (e.g., chr1, chrX); (2) Column 4 (start): Defines the starting genomic position of the TE insertion; (3) Column 5 (end): Defines the ending genomic position of the TE insertion; (4) Column 9 (TE info): Contains the identity of the TE, typically including the TE subfamily. Columns not used for core quantification (e.g., 2, 3, 6, 7, 8) can be filled with placeholder values but must be present to maintain the correct column order. All columns should be tab-delimited.
```
1       RepeatMasker    dispersed_repeat        1604    1768    35.8    +       .       ID=1;Target "Motif:L2c" 2993 3159
1       RepeatMasker    dispersed_repeat        3973    4154    25.9    -       .       ID=2;Target "Motif:MIRb" 46 240
1       RepeatMasker    dispersed_repeat        10184   10306   29.3    -       .       ID=3;Target "Motif:L1MC5" 7810 7936
1       RepeatMasker    dispersed_repeat        11130   11365   30.1    -       .       ID=4;Target "Motif:MIRb" 23 260
```
Complete parameters `-h`:
```sh
Usage: path/to/bin/HQuaTE
        -b  <BAM file (sorted)>
        -g  <TE annotation file (format as RepeatMasker.GFF)>
        -o  <Output prefix> (default: working directory)
        -f  <Minimum overlap requirement> (default: 0.5)
        -p  <Maximum likelihood value for assignment> (default: 0.8)
        -m  <Maximum iterations of the outer cycle> (default: 10)
        -M  <Maximum iterations of the inner cycle> (default: 10)
        -e  <Abnormal ERV detection> (default: Flase)
        -v  <Show version information>
        -h  <Show this help message>
```
## Advanced features
### 1. Cleaning the TE annotation file (gff)
In order to clean the silent TEs. A TE database is constructed from the long reads of RNA-seq in several domestic animals. Based on this database, types of silent TE from GFF file would be discarded. Then returning another GFF file for quantification. 
Presently, this function `GFFfilter` is limited to a selection of domestic animals, specifically pigs, cattle, sheep, goats, and chickens. For the other species, users could customize the TE annotation file to adjust the active TE subfamilies.
```sh
filterGFF -s <species> -g <TE.gff>
```

### 2. Detecting anomalous expression of ERVs
HQuaTE also comes with a python script that detects the anomalous  expression of ERVs by `-e`. It statistics the ratio of each ERV between RNA-seq and genome. Based on DBSCAN, the ERVs with anomalous value are deemed as exogenous.
For the specific parameters, Euclidean distance is used as the metric for computation. And the eps set to 3. Moreover, users could modify them in that python script file `abnormal_ERVs.py` to increase or decrease its sensitivity. Additionally, users could modify the textfile `ERVFamily` to detect the specific TE subfamilies.
```sh
HQuaTE -b <bam> -g <TE.gff> -e
```

### 3. Expected probability threshold
This parameter `-p` determines the assignment of a multi-mapping TE read indicated by its expected probability. Increasing this parameter yields more accurate assignments but extends processing time due to fewer multi-mapped reads being resolved per cycle.
```sh
HQuaTE -b <bam> -g <TE.gff> -p 0.8 
```

### 4. Minimum overlap requirement
Due to most TEs are predominantly active in fragment. Therefore, the default standard is relatively loose to prevent the loss of TE reads. Herein, Users have the flexibility to adjust the key parameters, overlapping rate `-f`, to manage the number of TE reads. This step ensures a balance between capturing a comprehensive representation of TE expression and maintaining the integrity of the TE reads.
```sh
# Reporting only those intersections where 50% of the read is overlapped by a TE.gff (default)
HQuaTE -b <bam> -g <TE.gff> -o <output> -f 0.5
# Reporting only those intersections where 100% of the read is overlapped by a TE.gff
HQuaTE -b <bam> -g <TE.gff> -o <output> -f 1
```

## Citing 
```
TBD
```

